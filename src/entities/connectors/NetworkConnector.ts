...
class MiniRpcProvider implements AsyncSendable {
  ...
  constructor(connector: NetworkConnector, chainId: number, url: string, batchWaitTimeMs?: number) {
    ...
    this.chainId = chainId ?? 204
    this.url = url ?? "https://opbnb-mainnet-rpc.bnbchain.org"
    const parsed = new URL(url)
    this.host = parsed.host ?? "opbnb-mainnet-rpc.bnbchain.org"
