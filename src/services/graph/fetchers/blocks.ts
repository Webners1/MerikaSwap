...
const fetcher = async (chainId = ChainId.BSC, query, variables = undefined) => {
  // @ts-ignore TYPE NEEDS FIXING
  return request(`${GRAPH_HOST[chainId]}/subgraphs/name/${BLOCKS[chainId]}`, query, variables)
}

// @ts-ignore TYPE NEEDS FIXING
export const getBlock = async (chainId = ChainId.BSC, variables) => {
...
// @ts-ignore TYPE NEEDS FIXING
export const getBlockDaysAgo = async (chainId = ChainId.BSC, days) => {
...
// @ts-ignore TYPE NEEDS FIXING
export const getBlocks = async (chainId = ChainId.BSC, variables) => {
  const { blocks } = await fetcher(chainId, blocksQuery, variables)
  return blocks
}

// @ts-ignore TYPE NEEDS FIXING
export const getMassBlocks = async (chainId = ChainId.BSC, timestamps) => {
...
export const getAverageBlockTime = async (chainId = ChainId.BSC) => {
